...
- **Chatwoot abaixo da 4.13:** o vínculo (etapa 9) cria o Agent Bot e precisa do `secret` que o Chatwoot devolve na criação, e esse campo só existe a partir da `v4.13.0` (no fork, `v4.13.0-fazer-ai-pro.75`). Numa versão anterior, como a `v4.7.0-fazer-ai.8`, a instalação corre inteira e quebra só no vínculo, com `Chatwoot agent_bot create: missing id/access_token/secret`. Tag abaixo de `v4.13.0` → **incompatível**: sinalize aqui e ofereça atualizar o Chatwoot, com backup do banco antes, antes da etapa 2. Se esse erro aparecer na etapa 9, a causa é esta: atualize e refaça o vínculo.
