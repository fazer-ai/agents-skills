...
Toda a config do fazer.ai agents (import do agente, vault, tenant-settings, KB, deployment/bind) é **exclusivamente via MCP tools**: elas carregam dry-run + audit + o fence de tenant. Um server MCP recém-adicionado não aparece sozinho na sessão aberta: ela precisa **recarregar o config**, e **como e quando muda por harness**. O Claude autentica na TUI (`/mcp`), que exige o server já carregado, então recarrega com **`/reload-plugins`** (reinício só se o reload não trouxer o server) **antes** de autenticar; Codex/Hermes autenticam por comando de CLI e reiniciam **depois**. Endpoint MCP do fazer.ai agents: **`https://agents.<seu-dominio>/api/v1/mcp`** — use o path completo `/api/v1/mcp`. A raiz `https://agents.<seu-dominio>` ou um `.../mcp` sem o `/api/v1` cai na SPA e o login OAuth falha com `invalid_target` (o servidor liga o token ao recurso canônico `.../api/v1/mcp`). Discovery em `docs/mcp.md`.

**Claude Code** (recarregue ANTES de autenticar):
1. **Adicione** (transport HTTP, com o path completo): `claude mcp add --transport http fazer-ai https://agents.<seu-dominio>/api/v1/mcp`. O server entra no config, mas **não** aparece na sessão atual nem no `/mcp` até a sessão recarregar.
2. **Recarregue:** o usuário digita **`/reload-plugins`** na mesma sessão (é comando da TUI, você não roda por ele). Agora o `/mcp` lista `fazer-ai` como **"Needs authentication"** (esperado, não é falha). **Fallback:** se depois do reload o `fazer-ai` não aparece no `/mcp`, reinicie a sessão (feche e reabra com `claude --resume` no mesmo dir, que mantém a conversa) e confira de novo.
3. **Autentique:** `/mcp` → `fazer-ai` → **Authenticate** → browser; o usuário loga com o admin do `/setup` (SUPER_ADMIN) e aprova os escopos (`mcp:read/write/admin`). Ao voltar **"Connected"**, as tools carregam **na mesma sessão, sem outro reload nem reinício**.
...
- **PARE e peça ao usuário pra completar o passo do harness dele** (Claude: **`/reload-plugins` → `/mcp` Authenticate**, com reinício via `claude --resume` só se o reload não trouxer o `fazer-ai`; Codex/Hermes: **`mcp login` → reiniciar**), confirmando o Authenticate/login **e** o reload ou reinício. Espere ele voltar. Esse é o **único** caminho.
...
- **Sinal de que você entrou no anti-padrão:** se você se pegou grepando `agents.controller.ts`, procurando `POST /api/v1/agents/import`, ou mintando uma API key pra "equivalente REST" porque "a tool não apareceu" → **PARE imediatamente** e peça o reload (Claude) ou o reinício (Codex/Hermes). Não existe "fallback REST transitório" para config do fazer.ai agents. **Idem pra achar uma rota/deeplink do console:** não baixe+grepe o bundle da SPA; as rotas que a skill usa estão nas refs (ex.: o deeplink de credencial em `08-agent-import.md` §2), e o bundle é minificado/hasheado (frágil).
